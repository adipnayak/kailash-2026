...
function ConnIcon({ status }: { status: 'good' | 'intermittent' | 'offline' }) {
  if (status === 'good')
    return <Icon name="wifi_tethering" size={12} className="text-emerald" aria-label="Good connectivity" />;
  if (status === 'intermittent')
    return <Icon name="wifi_tethering" size={12} className="text-sacred" aria-label="Intermittent connectivity" />;
  return <Icon name="warning" size={12} className="text-destructive" aria-label="No connectivity" />;
}
