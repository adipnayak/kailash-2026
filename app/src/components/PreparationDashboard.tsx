...
function StatusIcon({ status }: { status: ItemStatus }) {
  if (status === 'complete') {
    return <CircleCheck size={16} className="text-emerald-600 shrink-0" />;
  }
  if (status === 'pending') {
    return <Clock size={16} className="text-accent-foreground shrink-0" />;
  }
  ...
  return (
    <div
      className={[
        ...
        hasBlocking ? 'border-destructive/40' : allDone ? 'border-emerald-600/40' : 'border-border',
      ].join(' ')}
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          {category.optional && (
            <span className="rounded border border-muted-foreground/30 px-1.5 py-0.5 font-mono text-xs text-muted-foreground">
              optional
            </span>
          )}
          {hasBlocking && (
          ...
          {allDone && !hasBlocking && (
            <span className="rounded bg-emerald-600/10 px-1.5 py-0.5 font-mono text-xs text-emerald-600">
              complete
            </span>
          )}
        </div>
      ...
      <div className="h-0.5 bg-border">
        <div
          className={[
            ...
            allDone ? 'bg-emerald-600' : hasBlocking ? 'bg-destructive' : 'bg-accent',
          ].join(' ')}
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div ref={headerRef}>
          ...
          <div className="mt-4 h-1 rounded-full bg-border overflow-hidden">
            <motion.div
              className={['h-full rounded-full', blocking > 0 ? 'bg-destructive' : pct === 100 ? 'bg-emerald-600' : 'bg-accent'].join(' ')}
              initial={{ width: 0 }}
              animate={{ width: `${pct}%` }}
