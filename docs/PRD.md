...
8. **Diamox capsule safety warning (Reference > Medicines)**. Critical patient-safety addition. The existing regime calendar shows "half tab" doses (125 mg = half of a 250 mg tablet). This assumes splittable TABLETS. If a yatri's pharmacy filled the Rx with CAPSULES instead, splitting them is unsafe -- powder spills, dose inconsistent, and if it's an SR/ER capsule (Iopar SR / Diamox SR / Acetazolamide ER/XR/SR) splitting destroys the timed-release mechanism and dumps the full dose at once. The SR formulation is a different protocol entirely: ONCE daily 500 mg, not twice daily 125 mg.

   **Where**: insert new section in `app/src/lib/reference-data.ts` in the `medicines-and-diamox` article, BETWEEN the existing DIAMOX (ACETAZOLAMIDE) PROTOCOL table and the DIAMOX REGIME CALENDAR section. Positioned so yatris encounter the safety warning BEFORE they read the regime calendar's "half tab" numbers.

   **Blocks to insert** (in this order):

   1. `heading` "CAPSULES VS TABLETS - DO NOT SPLIT CAPSULES"

   2. `callout` (tone: critical) -- "Capsules cannot be safely split. Three problems: powder spills make dosing inconsistent; if it is an extended-release SR or ER capsule, breaking it destroys the timed-release mechanism and dumps the full dose at once; the capsule cannot be resealed accurately."

   3. `heading` "CHECK YOUR STRIP LABEL FIRST"

   4. `prose` -- "Before you take your first dose, read the strip the pharmacy gave you."

   5. `table` headers `["Strip says", "Form", "Protocol"]`:

      | Strip says | Form | Protocol |
      |---|---|---|
   ...
   6. `callout` (tone: warning) -- "If you ended up with SR or ER capsules instead of standard tablets, do NOT just take half. The regime calendar below ASSUMES standard 250 mg tablets. Confirm your formulation with the prescribing doctor before your first dose."

   **Optional DayCard addition** (DEFERRED to a future iteration, NOT in this PR): a tiny "TABLETS ONLY - DO NOT SPLIT CAPSULES" subline under the DIAMOX TODAY block on the DayCards. Keeping this iteration tight: Reference is the safety-info surface.

   **Files touched**:
   - **Edit**: `app/src/lib/reference-data.ts` only (insert 6 blocks in the `medicines-and-diamox` article)

   **Single small change, no parallelization required.** Haiku in worktree.
...
- 2026-06-22 v2.12 -- Diamox capsule safety warning. Critical patient-safety addition to Reference > Medicines article. Inserts new "CAPSULES VS TABLETS - DO NOT SPLIT CAPSULES" section BETWEEN the existing PROTOCOL Q&A table and the REGIME CALENDAR (so yatris see the safety warning BEFORE the regime's "half tab" numbers). 6 new blocks: critical-tone callout (three problems with splitting capsules), heading + prose for "check your strip label first", 4-row table mapping strip label (Iopar SR / Diamox SR / plain capsule / standard tablet / pre-split 125 mg tablet) to form and protocol, warning-tone callout for SR/ER formulation mismatch (SR is once-daily 500 mg, not twice-daily 125 mg). Single file edit (reference-data.ts); Haiku in worktree.
